...
- **50→30 임계값 실험을 이 baseline bag으로 재실행해 최종 결론을 냈다(기각)**: 처음엔
  라이브로 재시도할 때마다 실제 모션이 달라서 직접 비교가 안 됐는데, 같은 bag을 양쪽
  빌드(50 기본값 vs 30)에 그대로 재생해 공정 비교함 — `New Map created` 50→40회,
  `Fail to track local map!` 49→38회로 리셋 빈도는 줄었지만, **`start VIBA 1`은 양쪽 다
  0회로 동일**. 동일 입력에서도 결과가 같았으므로, 이 게이트가 유일한 병목이 아니라는
  게 확정됐다. **실험 코드는 원복 + 재빌드 완료.** 근본 원인은 임계값 자체보다, 리셋
  직후 로컬맵 매칭 품질이 이 bag의 모션/장면 조건에서 구조적으로 부족한 쪽에 더 가깝다.
...
재연결 문제는 근본 원인(브리지의 고정 데드라인 버그)을 찾아 완전히 고쳤다(slam-tx2#20,
PR #21 — `--recv-timeout` 기본값 10.0 그대로 재연결 0회 달성). bag 재생 자체가
비결정적이었다는 것도 발견해 `reliable_sensor_qos`로 고쳤다(`docs/bag-replay-determinism.md`
참고). 파랄락스 부족도 계측으로 배제했다. 하지만 **핵심 트래킹 문제(초기화 직후 즉시
실패)는 미해결로 남겨두고 세션을 마무리했다** — 자세한 최종 결론은
[docs/imu-init-debug.md](imu-init-debug.ko.md)의 "이후 조사 결과 (2026-09-21) - 결론" 참고.
이 글을 쓰는 시점까지, RViz에 라이브로 Path를 지속적으로 그리는 데는 성공하지 못했다.

**세션 결론**: 이 프로젝트 세션의 목적은 iPhone Xs Max + TX2 조합으로 제품을 만드는 게
아니라 SLAM 파이프라인의 동작을 이해하는 것이었다. 통제 가능한 레버(CPU, 크래시, 네트워크,
파랄락스, 전송 결정성)는 전부 찾아 고치거나 배제했다. 남은 원인 후보는 ORB-SLAM3 자체의
멀티스레드 실행 순서 비결정성과, 아직 검증 안 됐지만 유력한 롤링 셔터 왜곡 — 둘 다 이
하드웨어 조합의 근본적 특성에 가깝다고 판단해 조사를 일단 마무리했다.

## 다음에 할 만한 일

1. **(가장 유력, 미검증) 롤링 셔터 왜곡 확인** — 원본 특징점/매칭/파랄락스가 전부
   양호한데도 정밀한 재투영 일관성이 필요한 단계에서만 실패하는 패턴과 부합한다. 실제
   실패 순간(리셋 직전)의 프레임을 저장해 기하학적 왜곡(위/아래 행 간 시점 차이로 인한
   skew)이 보이는지 직접 확인할 것. 자세한 배경은 `docs/imu-init-debug.md` 참고.
2. **위 "SLAM 노드가 ROS2 그래프에서 사라지는" 문제 근본 원인을 찾을 것** — `/rviz`
   노드에서도 같은 증상이 관찰됨(재시작 없이는 subscription이 안 살아남음). gdb로
   재현해서 실제 죽는 스레드의 백트레이스를 잡을 것.
...
4. **(우선순위 낮음) Kalibr로 정식 Tbc/IMU 캘리브레이션** — 지금 가장 자주 겪는 실패는
   IMU 초기화 이전의 순수 시각 기반 경로에서 일어나서 Tbc/IMU 정밀도와 무관하다. IMU
   초기화 자체가 안정적으로 시작될 때가 오면(1번이 해소된 뒤) 그때 다시 고려할 것.
...
- [bag-replay-determinism.md](bag-replay-determinism.ko.md) — bag 재생 비결정성 원인/해결 (#20, PR #21)
