...
1. **(2026-09-21 확인) 임계값 자체는 병목이 아니었다 — 초기 맵 품질을 개선하는 쪽으로
   방향 전환할 것.** baseline bag으로 50 vs 30을 동일 입력에서 공정 비교했더니
   `start VIBA 1`이 양쪽 다 0회였다(위 참고). 다음으로 시도해볼 것: 리셋 직후 2-view
   초기화가 더 넓은 파랄락스를 갖도록 유도(초기화 대기 프레임 수/파랄락스 최소치 조정),
   초기 맵의 아웃라이어 제거를 강화, 또는 애초에 이 baseline bag의 모션/장면 자체가
   IMU 초기화에 부적합할 가능성(더 밝고 텍스처 많은 공간에서 새 baseline bag을 하나 더
   녹화해 같은 방식으로 비교).
