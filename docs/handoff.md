...
RViz 라이브 확인 절차는 저장소 루트 [README.md](../README.md)에 다이어그램과 함께 정리돼 있다 —
...
**절차 + 노드/스크립트 연결 다이어그램은 [README.md](../README.md)에 정리돼 있다** — 여기서
중복 설명하지 않는다. 더 세세한 배경(각 단계에서 뭘 왜 그렇게 하는지)이 필요하면
`docs/live-e2e-validation.md`의 "0. 사전 준비"~"4. 라이브 vs 재생 비교" 참고.
